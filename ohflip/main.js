...
goals.push({text: "Reach a height of 50 ft", func: ReachedHeight, param: 50});
...
goals.push({text: "Reach a height of 100 ft", func: ReachedHeight, param: 100});
...
{
    // Touch events carry no pageX/pageY of their own - the coordinates live on
    // the individual touches, so reading event.pageX here gave NaN on mobile.
    // getBoundingClientRect also maps through the CSS scaling the canvas now
    // uses to fit narrow screens.
    let src = event.changedTouches ? event.changedTouches[0] : event;
    if (!src)
    {
        return;
    }
    ...
    touchX = (src.clientX - rect.left) * (canvas.width / rect.width);
    touchY = (src.clientY - rect.top) * (canvas.height / rect.height);
}
...
function GameLoop(curTime)
{
    ...
    // Reading a popup must not cost the player their run - the buttons are
    // reachable mid-game, and without this the bounce height decays away while
    // nobody is touching the screen. Drawing continues so the frame behind the
    // popup stays correct, and lastFrameTime is updated above so closing the
    // popup does not release one huge catch-up step.
    if (!AnyModalOpen())
    {
        UpdateUI(dt);
        UpdatePlayer(dt);
...
function DrawTrampoline()
{
    ...
    // Cache code on the grass, framed by the poles and the mat. Drawn inside
    // the camera transform so it belongs to the world rather than the HUD, and
    // before the mesh so the trampoline shake never rides across it. Same
    // black-backing-plus-orange treatment as the title.
    let codeTxt = "GCBVZEN";
    DrawText(codeTxt, 0, 50, -5*Math.PI/180.0, 40, "center", "#000");
    DrawText(codeTxt, -3, 46, -5*Math.PI/180.0, 40, "center", "#FF9600");
...
function DrawUI()
{
    ...
    if (mainMenu)
    {
        ...
        let cacheTxt = "Mystery Cache GCBVZEN";
        DrawText(cacheTxt, (canvas.width*0.5), 315, -5*Math.PI/180.0, 36, "center", "#000");
        DrawText(cacheTxt, (canvas.width*0.5) - 3, 311, -5*Math.PI/180.0, 36, "center", "#FF9600");
...
// Which goal reveals the payoff, counted the way the HUD shows it (Goal #1 is
// the first).
const SOLUTION_LEVEL = 10;

const payloadSeed = 0x7A3B19C5;
const payloadData = "xsgLA5VyZ2FZI/Y1FG0kf62xv8sVLezEA7iXE4guVk5Oyw+Be5SXiA==";
...
function Unpack(data, seed)
{
    ...
    for (let i = 0; i < raw.length; i++)
    {
        ...
        bytes[i] = raw.charCodeAt(i) ^ ((s >>> 24) & 255);
    }
...
function WireModal(modal, openButton)
{
    // Clicking the backdrop closes; clicking inside the card does not, but the
    // event still must not reach the game.
    OnActivate(modal, () => SetModalOpen(modal, false));
    SwallowEvents(modal.querySelector(".modalCard"));

    modal.querySelectorAll(".modalClose").forEach(button =>
    {
        OnActivate(button, () => SetModalOpen(modal, false));
    });

    if (openButton)
    {
        OnActivate(openButton, () => SetModalOpen(modal, true));
    }

    return modal;
}

let instructionsModal = WireModal(document.getElementById("instructionsModal"), document.getElementById("instructionsBtn"));
let creditsModal = WireModal(document.getElementById("creditsModal"), document.getElementById("creditsBtn"));
let cacheModal = WireModal(document.getElementById("cacheModal"), null);

document.addEventListener("keydown", e =>
{
    if (e.key === "Escape")
    {
        [instructionsModal, creditsModal, cacheModal].forEach(modal => SetModalOpen(modal, false));
    }
});

// Numbers in the instructions come from the goal list itself so they cannot go
// stale when goals are added or removed.
document.getElementById("levelCount").textContent = goals.length;
document.getElementById("solutionLevel").textContent = SOLUTION_LEVEL;
document.getElementById("cacheLevel").textContent = SOLUTION_LEVEL;

// Remembered separately from goalIdx so that resetting the goals - which the
// game offers once all of them are done - does not take the coordinates back
// from a player who already earned them.
function SolutionEarned()
{
    return goalIdx >= SOLUTION_LEVEL || localStorage.getItem("ohflip.cacheEarned") === "1";
}

// Keeps the promise made in the reveal popup: once earned, the coordinates stay
// available from the instructions.
function RefreshInstructionsCache()
{
    let element = document.getElementById("instructionsCache");
    element.innerHTML = "";

    if (SolutionEarned())
    {
        let heading = document.createElement("p");
        heading.innerHTML = "<b>You have already earned the coordinates:</b>";
        let message = document.createElement("div");
        message.className = "cacheCoords";
        message.textContent = Unpack(payloadData, payloadSeed);
        element.appendChild(heading);
        element.appendChild(message);
    }
}

function RevealCacheIfEarned()
{
    if (goalIdx === SOLUTION_LEVEL)
    {
        localStorage.setItem("ohflip.cacheEarned", "1");
        // Filled in here rather than at load so the popup's text is not sitting
        // in the DOM before it has been earned.
        document.getElementById("cacheMessage").textContent = Unpack(payloadData, payloadSeed);
        RefreshInstructionsCache();
        SetModalOpen(cacheModal, true);
    }
}
